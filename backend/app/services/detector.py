...
def _has_external_subject(text: str) -> bool:
    ...
    lower = text.lower()
    internal = {"we ", "our ", "ocf ", "the organization", "the team"}
    external_hints = {
...
async def detect_llm(anchors: list[ParsedAnchor], api_key: str) -> list[DetectedSignal]:
...
It is NOT: an activity, an event, a deliverable, or a headcount statistic.

For each paragraph, respond with JSON:
{
  "is_signal": true/false,
  "is_context_signal": true/false,  // true if this is an external environmental change (law, policy by others)
  "level": "L1"/"L2"/"L3"/null,
...
Level definitions:
- L1: Confirmed/institutionalized (adopted, enacted, formally established)
- L2: Committed intent or trial (agreed to start, planning, piloting)
- L3: Awareness/interest only (discussing, growing awareness, expressing interest)"""
